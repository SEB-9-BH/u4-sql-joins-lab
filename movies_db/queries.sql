...
SELECT title, name FROM users_movies 
INNER JOIN users ON users.user_id = users_movies.user_id 
INNER JOIN movies ON movies.movie_id = users_movies.movie_id;



-- 3. List the movies with the number of favorites they have.
SELECT title, COUNT(*) FROM users_movies 
JOIN movies on movies.movie_id = users_movies.movie_id
GROUP BY users_movies.movie_id , movies.title;
...
SELECT users.name, directors.name, COUNT(*) FROM users_movies
JOIN movies ON movies.movie_id = users_movies.movie_id
