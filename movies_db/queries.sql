...
SELECT m.title AS movie_title, COUNT(um.user_id) AS favorites_count
FROM movies m
LEFT JOIN users_movies um ON m.movie_id = um.movie_id
GROUP BY m.movie_id, m.title
ORDER BY favorites_count DESC, m.title;
...
SELECT u.name AS user_name, d.name AS director_name, COUNT(um.movie_id) AS favorite_count
FROM users u
JOIN users_movies um ON u.user_id = um.user_id
JOIN movies m ON um.movie_id = m.movie_id
JOIN directors d ON m.director_id = d.director_id
GROUP BY u.user_id, u.name, d.director_id, d.name
ORDER BY u.name, favorite_count DESC, d.name;
